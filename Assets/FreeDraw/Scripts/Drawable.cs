...
// BRUSH TYPES. Implement your own here
        ...
        public void CrayonBrush(Vector2 world_point)
        {
            Vector2 pixel_pos = WorldToPixelCoordinates(world_point);
            ...
            {
                // If this is the first time we've ever dragged on this image, simply colour the pixels at our mouse position
                MarkPixelsToColour(pixel_pos, BrushTexture.texture, Pen_Colour);
            }
            else
            {
                // Colour in a line from where we were on the last update call
                ColourBetween(previous_drag_position, pixel_pos, BrushTexture.texture, Pen_Colour);
            }
            ApplyMarkedPixelChanges();
        ...
        public void SetCrayonBrush()
        {
            // PenBrush is the NAME of the method we want to set as our current brush
            current_brush = CrayonBrush;
        }
        ...
        public void ColourBetween(Vector2 start_point, Vector2 end_point, Texture2D pen_type, Color color)
        {
            // Get the distance from start to finish
            float distance = Vector2.Distance(start_point, end_point);
            Vector2 direction = (start_point - end_point).normalized;
            ...
            {
                cur_position = Vector2.Lerp(start_point, end_point, lerp);
                MarkPixelsToColour(cur_position, pen_type, color);
            }
        }
        public void MarkPixelsToColour(Vector2 center_pixel, Texture2D pen_type, Color color_of_pen)
        {
            ...
            for (int x = center_x - pen_type.width / 2; x <= center_x + pen_type.width / 2; x++)
            {
                // Check if the X wraps around the image, so we don't draw pixels on the other side of the image
                if (x >= (int)drawable_sprite.rect.width || x < 0)
                    continue;

                for (int y = center_y - pen_type.height / 2; y <= center_y + pen_type.height / 2; y++)
                {
                    if (pen_type.GetPixel(x - (center_x - pen_type.width / 2), y - (center_y - pen_type.height / 2)).a > 0.0f)
                    {
